...
for i in range(numFrames):
    ax.set_ylim(1.1*min_psi_y, 1.1*max_psi_y)
    line, =ax.plot(y_points, psiPlots[i], lw=1)
    plt.title('psi Time '+str(interFrameT*i))
    plt.savefig('./gifs/psi/n{0}/{1:04d}.png'.format(fMode, i))
    plt.cla()

for i in range(numFrames):
    ax.set_ylim(1.1*min_xx_y, 1.1*max_xx_y)
    line, =ax.plot(y_points, xxPlots[i], lw=1)
    plt.title('Cxx Time '+str(interFrameT*i))
    plt.savefig('./gifs/cxx/n{0}/{1:04d}.png'.format(fMode, i))
    plt.cla()

for i in range(numFrames):
    ax.set_ylim(1.1*min_xy_y, 1.1*max_xy_y)
    line, =ax.plot(y_points, xyPlots[i], lw=1)
    plt.title('Cxy Time '+str(interFrameT*i))
    plt.savefig('./gifs/cxy/n{0}/{1:04d}.png'.format(fMode, i))
    plt.cla()

for i in range(numFrames):
    ax.set_ylim(1.1*min_yy_y, 1.1*max_yy_y)
    line, =ax.plot(y_points, yyPlots[i], lw=1)
    plt.title('Cyy Time '+str(interFrameT*i))
    plt.savefig('./gifs/cyy/n{0}/{1:04d}.png'.format(fMode, i))
    plt.cla()
