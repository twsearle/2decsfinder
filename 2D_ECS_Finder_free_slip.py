...
#   2D ECS finder
#
#   Last modified: Tue 15 Apr 15:07:26 2014
#
#-----------------------------------------------------------------------------
...
def solve_eq(xVec):
    ...
    # The 'no vorticity slip ' boundary conditions du/dy(+-1) = 1, v(+-1) = 0
    ...
    # du/dy(+-1) = d2PSI/dy2(+-1) = 0 for all except zeroth mode
    for k in range (2*N+1):
        if k == N: continue # skip the 0th component 
        residualsVec[k*M + M-4] = dot(DOUBDERIVTOP, PSI[k*M:(k+1)*M])
        residualsVec[k*M + M-3] = dot(DOUBDERIVBOT, PSI[k*M:(k+1)*M])
    del k

    # du/dy(+-1) = d2Psi/dy20(+-1) = 1 set the zeroth mode to be 1, the constant
    # vorticity part of the boundary condition.
    residualsVec[N*M + M-3] = dot(DOUBDERIVTOP, (PSI[N*M:(N+1)*M])) - 1 
    residualsVec[N*M + M-2] = dot(DOUBDERIVBOT, (PSI[N*M:(N+1)*M])) - 1 

    # Psi0(-1) = 0
    # an extra boundary condition to set the scale of PSI - otherwise only its
    # derivatives matter and there is nothing to set zeroth Chebyshev mode.
...
def find_jacobian(x):
    ...
    # The 'no vorticity slip ' boundary conditions du/dy(+-1) = 1, v(+-1) = 0
    # So this means that the dx vector  must have ddu/dy(+-1) = 0, dv(+-1) = 0
    # or d2dpsi/dy(+-1) = 0 and ddpsi/dx (+-1) = 0


    # Apply BC to zeroth mode
    # d2psi0/dy = const
    jacobian[N*M + M-3, 0:4*vecLen + 1] = \
        concatenate( (zeros(N*M), DOUBDERIVTOP, zeros(N*M+3*vecLen+1)) )
    jacobian[N*M + M-2, 0:4*vecLen + 1] = \
        concatenate( (zeros(N*M), DOUBDERIVBOT, zeros(N*M+3*vecLen+1)) )
    # psi(-1) = const 
    ...
    for n in range(2*N+1):
        ...
        jacobian[n*M + M-4, 0:4*vecLen + 1] = \
            concatenate( (zeros(n*M), DOUBDERIVTOP, zeros((2*N-n)*M+3*vecLen+1)) )
        jacobian[n*M + M-3, 0:4*vecLen + 1] = \
            concatenate( (zeros(n*M), DOUBDERIVBOT, zeros((2*N-n)*M+3*vecLen+1)) )
    del n
...
doubleDY = dot(singleDY, singleDY)
DOUBDERIVTOP = zeros((M), dtype='complex')
DOUBDERIVBOT = zeros((M), dtype='complex')
for j in range(M):
    DOUBDERIVTOP[j] = dot(BTOP, doubleDY[:,j]) 
    DOUBDERIVBOT[j] = dot(BBOT, doubleDY[:,j])
del j
